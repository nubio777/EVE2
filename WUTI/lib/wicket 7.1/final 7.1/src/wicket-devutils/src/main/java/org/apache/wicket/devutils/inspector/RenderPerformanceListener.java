...
/**
 * A listener that adds a special {@link org.apache.wicket.behavior.Behavior} that measures the time needed by a component to
 * render itself. {@link org.apache.wicket.MarkupContainer}'s render includes the time for rendering its children so
...
public class RenderPerformanceListener implements IComponentInstantiationListener
{
	...
	/**
	 * A {@link org.apache.wicket.behavior.Behavior} that sets the current time in the meta data of the component before its
	 * render starts and use it to calculate how long it took. The collected data is logged as debug
	 * and also can be read from the meta data with key {@link org.apache.wicket.markup.html.debug.PageView#RENDER_KEY}.
